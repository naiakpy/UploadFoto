...
if(empty($foto)){ // Jika user tidak memilih file foto pada form
  ...
  $sql = $pdo->prepare("UPDATE siswa SET nik=:nik, nama=:nama, jenis_kelamin=:jk, telp=:telp, alamat=:alamat WHERE id=:id");
  ...
  if(move_uploaded_file($tmp, $path)){ // Cek apakah gambar berhasil diupload atau tidak
    ...
    // Proses ubah data ke Database
    $sql = $pdo->prepare("UPDATE siswa SET nik=:nik, nama=:nama, jenis_kelamin=:jk, telp=:telp, alamat=:alamat, foto=:foto WHERE id=:id");
    $sql->bindParam(':nik', $nik);
    $sql->bindParam(':nama', $nama);
    $sql->bindParam(':jk', $jenis_kelamin);
    $sql->bindParam(':telp', $telp);
